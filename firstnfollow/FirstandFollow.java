...
class FirstandFollow {
    ...
    public static void main(String args[]) throws IOException {
        ...
        for (i = 0; i < ntlen; i++) System.out.println("First Set of " + nt.charAt(i) + " is {" + removeDuplicates(fst[i]) + "}");
        ...
        for (i = 0; i < ntlen; i++)
            System.out.println("Follow Set of " + nt.charAt(i) + " is {" + removeDuplicates(flw[i]) + "}");
    }
    static String first(int i) {
        ...
        for (j = 0; j < grmr[i].length; j++) //number of productions
        {
            ...
            //when nonterminal has epsilon production
            {
                for (l = 0; l < ntlen; l++) //finding nonterminal
                {
                    if (grmr[i][j].charAt(k) == ntermnl[l])
                    //for nonterminal in first set
                    {
                        str = first(l);
                ...
                //entering each production
                {
                        ...
                        //finding the nonterminal whose follow set is to be found 
                    {
                                ...
                                //if it is the last terminal/non-terminal then follow of current non - terminal 
                            {
                                    ...
                                    //first of next non-terminal otherwise (else later...)
                                    {
                                        ...
                                        for (n = 0; n < chr.length; n++) {
                                            ...
                                            //if first includes epsilon
                                            {
                                                ...
                                                temp = temp + chr[n];
                                            //include whole first set except epsilon
                                        }
                                        found = 1;
                                    }
                                }
                                if (found != 1)
                                    temp = temp + pro[l + 1];
                                //follow set will include terminal(else is here)
                            }
                        }
                }
            }
